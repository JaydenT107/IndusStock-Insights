...
def line_chart(data,name,sday,eday,date_format, date_format_2 ,new_title=None, add_trendline = False):
    ...
    if add_trendline == True:
        ...
        fig2.add_shape(type = 'line', x0 = line_start_date, y0 = line_start_value, x1 = line_end_date, y1 = line_end_value, line = dict(color = line_color_2, width = 2, dash = 'solid'))
        fig2.add_shape(type = 'line', x0 = line_start_date, y0 = line_start_high, x1 = line_end_date, y1 = line_end_high, line = dict(color = 'yellow', width = 2, dash = 'dash'))
        fig2.add_shape(type = 'line', x0 = line_start_date, y0 = line_start_low, x1 = line_end_date, y1 = line_end_low, line = dict(color = 'yellow', width = 2, dash = 'dash'))
        fig2.add_annotation(y = line_start_value, x = line_start_date, showarrow = True, text = f"{line_start_value}", ax = random_ax1 , ay = random_ax2, borderwidth = 0.1, arrowcolor = 'yellow', bgcolor = 'black')
        fig2.add_annotation(y = line_end_value, x = line_end_date, showarrow = True, text = f"{line_end_value}", ax = random_ax2 , ay = random_ax1, borderwidth = 0.1, arrowcolor = 'yellow', bgcolor = 'black')
...
def second_part(s3 = s3client, sector = sector):
    ...
    volume_response = s3.get_object(Bucket = 'stocksectordata' , Key = f'{sector}/volume_description.json')
    ...
    with col1:
        ...
        cdate_list = list(reversed(date_list))
        ...
        st.markdown(f"<h1 style='font-size: 20px; color:white;'>" f"<span style='color: yellow;'>{f_close_description[0]}</span> "  f"{' '.join(f_close_description[1:3])} "  f"<span style='color: yellow;'>{f_close_description[3]} {f_close_description[4]} {f_close_description[5]}</span> " f"{' '.join(f_close_description[6:])}</h1>",  unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.write(" ")     
        st.write(" ")  
        st.write(volume_description.split(" "))
        st.markdown(f"<h1 style='font-size: 20px; color:white;'>"
