...
def volatility_chart(data,name,sday,eday,date_format, date_format_2,new_title=None):
    ...
    fig.add_shape(type = 'rect',y0 = 0 , x0 = filtered_data_2.iloc[-1]['Date'] , x1 = filtered_data_2.iloc[0]['Date'], y1 = filtered_data_2['Volatility'].max(),fillcolor="rgba(0,100,80,0.2)", line = dict(width = 0))
    fig.update_layout(
    barmode = 'stack',
    dragmode = False,
    title = 'Volatility',
    title_font=dict(size=24, family='Soin Sans Pro', color='white'),
    legend=dict( x=0,  y=0.98, title = None, borderwidth = 0.3, font = dict(size = 10))
    )
